...
def downloadFile(file_name, fileserver_name, tcp_file_server_socket):
    try:
        ...
        if file_server_req_result == "FSP/1.0 Success":
            ...
            with open(os.path.basename(file_name), "wb") as dest_file:
                while True:
...
parser.add_argument(
    "-n", help="Nameserver ip address", dest="nameserver_ip", required=True
)
parser.add_argument(
    "-f", help="File to download SURL", dest="download_file_surl", required=True
)
...
if nameserver_recieved == "ERR Syntax" or nameserver_recieved == "Err Not Found":
    ...
    if file_name == "*":
            ...
            for file in files_to_download:
                if file != "":
                    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as newsock:
                        ...
                        downloadFile(file,fileserver_name,newsock)
        except socket.timeout:
        ...
        try:
             with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as newsock:
                newsock.connect((file_server_ip, int(file_server_port)))
                downloadFile(file_name, fileserver_name, newsock)
        except socket.timeout:
            print("Fileserver not responding...", file=sys.stderr)
            sys.exit(1)
        except Exception as e:
